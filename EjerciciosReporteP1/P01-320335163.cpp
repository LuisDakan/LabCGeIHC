...
const float wait = period / (float)COLORS;
...
vector<tuple<float,float,float>> rndnums() 
{
	srand(time(NULL));
	vector<tuple<float,float,float>> colors(COLORS);
	for (int i = 0;i <COLORS;i++) {
		
		auto a=(float)((rand() % 101)/100.0);
		auto b= (float)((rand() % 101) / 100.0);
		auto c= (float)((rand() % 101) / 100.0);
		colors[i] = make_tuple( a,b,c );
	}
	return colors;
}
...
int main()
{
	...
	vector<tuple<float,float,float>> colors=rndnums();
	if (!glfwInit())
	{
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		glUseProgram(0);
		this_thread::sleep_for(chrono::duration<float>(wait));
		i = (i + 1) % COLORS;
